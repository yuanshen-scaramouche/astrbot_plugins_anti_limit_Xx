...
7. **Important**: Import `logger` from `astrbot.api` (not `logging` module)
8. **Important**: Import `MessageEventResult` and `StarTools` for modern AstrBot v4.0+
...
- Log exceptions using `logger` from `astrbot.api` (not `print` or `logging`)
...
- **Use `event.set_result(MessageEventResult().message().use_t2i(False))` for command responses** (NOT `yield event.plain_result()`)
- Respect permissions using `PermissionType` constants
- Keep command handlers focused, delegate complex logic to helpers
- Use `StarTools.get_data_dir()` for plugin data directory (NOT `self.get_data_dir()`)
...
class AntiRepeatPlugin(Star):
    ...
    async def Xxhelp(self, event: AstrMessageEvent):
        event.set_result(MessageEventResult().message("Help message").use_t2i(False))
```
...
- **CRITICAL**: Use `StarTools.get_data_dir()` for plugin data directory (NOT `self.get_data_dir()`)
- **CRITICAL**: Use `event.set_result(MessageEventResult().message().use_t2i(False))` for command responses (NOT `yield event.plain_result()`)
- **CRITICAL**: Use `logger` from `astrbot.api` for all logging operations (NOT `logging` module)
- **CRITICAL**: Import `MessageEventResult` from `astrbot.api.event` for creating response messages
- Plugin requires AstrBot v4.0+ API compatibility
